  ...
  return (
    <nav className="bg-white bg-opacity-60 backdrop-blur-2xl flex items-center justify-between p-3 mb-[1rem] sticky top-0 z-20 font-semibold">
      <div className="flex ml-6">
        <NavLink to="/" className="cursor-pointer max-md:hidden">
          Logo
        </NavLink>
        <button
          onClick={() => sideDisplayHandler(true)}
        ...
        <ul className="ml-20 flex items-center gap-2 gap-x-8 max-md:hidden">
          <NavLink
            to="/products"
            className="cursor-pointer hover:text-green-700 hover:scale-[110%] transition-all">
            Products
          </NavLink>
          <NavLink
            to="/documents"
    ...
    <div className="flex gap-2 gap-x-4">
      ...
      <div className="relative">
        <UserCircleIcon
          ...
          className="w-[2.4rem] cursor-pointer"
        />
        {isOpenOption && (
          <NavOption display={isOpenOption} displayHandler={setIsOpenOption} />
        )}
      </div>
    </div>
  );
...
const NavOption = ({ display, displayHandler }) => {
  ...
  return (
    <ul
      ...
      className={`flex-col w-[12rem] absolute rounded-lg overflow-hidden shadow-customeFour right-1 top-10 border-[1px] border-zinc-200 font-normal z-20 flex ${
        display ? "flex" : "hidden"
      }`}>
      {role ? <UsersOptions /> : <AdminOptions />}
      <li
        onClick={logoutFn}
        className="bg-white border-t-[1px] border-zinc-200 flex gap-2 justify-between items-center p-2 cursor-pointer hover:bg-zinc-100">
        Log out
  ...
  return (
    <>
      <NavLink
        ...
        className="bg-white p-2 py-3 cursor-pointer hover:bg-zinc-100">
        <p>{username}</p>
...
const AdminOptions = ({ username }) => {
  return (
    <>
      <NavLink
        to="/dashboard"
        className="bg-white flex gap-2 justify-between items-center p-2 cursor-pointer hover:bg-zinc-100">
        Dashboard
        <PresentationChartLineIcon className="w-[1.5rem]" />
      </NavLink>
      <NavLink
        to="/dashboard/products"
        className="bg-white border-t-[1px] flex gap-2 justify-between items-center p-2 cursor-pointer hover:bg-zinc-100">
        Prodcuts
        <ShoppingBagIcon className="w-[1.5rem]" />
      </NavLink>
      <NavLink
...
const AuthenticateBtns = () => {
  return (
    <div className="flex gap-2 max-lg:gap-0 max-lg:rounded-lg overflow-hidden">
      <NavLink
        to="/signup"
        className="h-[3rem] border-2 border-green-400 rounded-xl p-2 flex gap-2 items-center lg:hover:bg-slate-100 max-lg:border-none max-lg:rounded-none max-lg:bg-zinc-200 max-lg:px-4">
        <p className="max-lg:hidden">Create an account</p>
        <UserPlusIcon strokeWidth={2} className="w-6 rounded-full" />
      </NavLink>
      <NavLink
        to="/login"
        className="h-[3rem] bg-green-400 rounded-xl p-1 px-2 flex items-center gap-2 hover:bg-green-500 max-lg:rounded-none max-lg:px-5">
